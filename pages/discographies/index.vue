<template>
  <div>
    <base-header class="pb-6" type="">
      <div class="row align-items-center py-4">
        <div class="col-lg-8 col-3">
          <h6 class="h2 d-inline-block mb-0">가사/콜</h6>
          <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
            <ol class="breadcrumb breadcrumb-links">
              <li class="breadcrumb-item"><nuxt-link to="/"><i class="fas fa-home mr-2"></i>홈</nuxt-link></li>
              <li class="breadcrumb-item active" aria-current="page">가사/콜</li>
            </ol>
          </nav>
        </div>
        <div class="col-lg-1 col-3 p-0 text-right" v-if="this.$store.state.authUser">
          <base-button outline type="default" @click="$router.push('/discographies/edit')" v-if="['editor', 'admin'].includes(this.$store.state.authUser.role)">추가</base-button>
        </div>
    ...
    <div class="container-fluid mt--6">
      <div class="row">
        <div class="col-lg-4 col-12 cards" v-for="song in songFiltered" :key="song.title">
          <nuxt-link :to="`/discographies/${encodeURIComponent(song.title)}`">
            <card>
              <!-- Card body -->
              <div class="row align-items-center">
                <div class="col-auto p-2">
                   <img v-lazy="song.image" v-if="song.image"
  ...
  export default {
    head () {
      return {
        title: '가사/콜',
        meta: [
          { name: 'og:title', content: '가사/콜', hid: 'og:title', template: chunk => `${chunk} - 부시로드 성우 정보` },
    ...
    methods: {
      ...
      async onLoad() {
        this.songList = await this.$axios.$get(`/api/discography/list`)
        this.songFiltered.push(...this.paginatedList)
      },
      async onType() {
