    ...
    <base-header class="pb-6" type="">
      <div class="row align-items-center py-4">
        <div class="col-lg-8 col-3">
          <h6 class="h2 d-inline-block mb-0">어썰트 릴리 가사/콜</h6>
          <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
            <ol class="breadcrumb breadcrumb-links">
              ...
              <li class="breadcrumb-item active" aria-current="page">어썰트 릴리</li>
            </ol>
          </nav>
        </div>
        <div class="col-lg-1 col-3 p-0 text-right" v-if="this.$store.state.authUser">
          <base-button outline type="default" v-if="['editor', 'admin'].includes(this.$store.state.authUser.role)" @click="$router.push('/discographies/edit?project=assaultlily')">추가</base-button>
        </div>
  ...
  export default {
    head () {
      return {
        title: '어썰트 릴리 가사/콜',
        meta: [
          { name: 'og:title', content: '어썰트 릴리 가사/콜', hid: 'og:title', template: chunk => `${chunk} - 부시로드 성우 정보` },
    ...
    methods: {
      ...
      async onLoad() {
        this.songList = await this.$axios.$get(`/api/discography/list/assaultlily`)
        this.songFiltered.push(...this.paginatedList)
      },
      async onType() {
