    ...
    <base-header class="pb-6" type="">
      <div class="row align-items-center py-4">
        <div class="col-lg-8 col-3">
          <h6 class="h2 d-inline-block mb-0">Re버스 for you 굿즈</h6>
          <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
            <ol class="breadcrumb breadcrumb-links">
              ...
              <li class="breadcrumb-item active" aria-current="page">Re버스 for you</li>
            </ol>
          </nav>
        </div>
        <div class="col-lg-1 col-3 p-0 text-right" v-if="this.$store.state.authUser">
          <base-button outline type="default" v-if="['editor', 'admin'].includes(this.$store.state.authUser.role)" @click="$router.push('/goods/edit?project=rebirth')">추가</base-button>
        </div>
              ...
              <div class="row align-items-center">
                <div class="col-auto p-2">
                  <img v-lazy="goods.image"
                       class="img-fluid" v-if="goods.image"
                ...
                <div class="col ml--2">
                  ...
                  <p class="text-sm text-muted mb-0">{{ goods.japaneseName }}</p>
                  <span>
...
<script>
  ...
  export default {
    head () {
      return {
        title: 'Re버스 for you 굿즈',
        meta: [
          { name: 'og:title', content: 'Re버스 for you 굿즈', hid: 'og:title', template: chunk => `${chunk} - 부시로드 성우 정보` },
    ...
    methods: {
      ...
      async onLoad() {
        this.goodsList = await this.$axios.$get(`/api/goods/list/rebirth`)
        this.goodsFiltered.push(...this.paginatedList)
      },
      async onType() {
