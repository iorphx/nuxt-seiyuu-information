    ...
    <base-header class="pb-6" type="">
      <div class="row align-items-center py-4">
        <div class="col-lg-8 col-3">
          <h6 class="h2 d-inline-block mb-0">D4DJ 굿즈</h6>
        ...
        <div class="col-lg-1 col-3 p-0 text-right" v-if="this.$store.state.authUser">
          <base-button outline type="default" v-if="['editor', 'admin'].includes(this.$store.state.authUser.role)" @click="$router.push('/goods/edit?project=d4dj')">추가</base-button>
        </div>
  ...
  export default {
    head () {
      return {
        title: 'D4DJ 굿즈',
        meta: [
          { name: 'og:title', content: 'D4DJ 굿즈', hid: 'og:title', template: chunk => `${chunk} - 부시로드 성우 정보` },
    ...
    methods: {
      ...
      async onLoad() {
        this.goodsList = await this.$axios.$get(`/api/goods/list/d4dj`)
        this.goodsFiltered.push(...this.paginatedList)
      },
      async onType() {
